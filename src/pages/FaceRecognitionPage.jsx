...
export default function FaceRecognitionPage({
  ...
  // ===== GPS ===== (DIUBAH: sampleCount default = 2)
  const getLocationSamples = useCallback(
    (sampleCount = 2, intervalMs = 1200) => { // <<< DIUBAH dari 3 ke 2
      return new Promise((resolve, reject) => {
        ...
        let completed = 0;
        let timedOut = false;
        let totalTimeout = 20000;

        const timeoutId = setTimeout(() => {
          timedOut = true;
          if (samples.length >= 2) { // <<< DIUBAH dari 3 ke 2
            resolve(samples);
          } else {
            reject(new Error(`Timeout: hanya ${samples.length} sampel terkumpul (minimal 2)`)); // <<< DIUBAH
          }
        }, totalTimeout);
  ...
  // ===== Proses utama absensi ===== (DIUBAH: sampleCount = 2)
  const processAttendance = useCallback(async () => {
    ...
    try {
      ...
      const samples = await getLocationSamples(2, 1200); // <<< DIUBAH dari 5 ke 2
      if (samples.length < 2) { // <<< DIUBAH dari 5 ke 2
        throw new Error('Gagal mengambil cukup sampel GPS (minimal 2)'); // <<< DIUBAH
      }
